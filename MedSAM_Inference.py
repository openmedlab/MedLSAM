...
for id in trange(len(nii_pathes)):
    ...
    try:
        ...
        for key in config_file['data']['fg_class']:
            ...
            axes[1].axis('off')
            # save figure
            fig.savefig(join(config_file['data']['seg_png_save_path'], '{0}_{1}_{2}_{3}.png'.format(os.path.basename(args.config_file).replace('test_','').replace('.txt',''), id, key, config_file['weight']['vit_load_path'].split('/')[-1].split('_')[0])))
            # close figure
            plt.close(fig)
